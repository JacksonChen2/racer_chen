...
int HGrid::getGridCount() const {
  return static_cast<int>(grid1_->grid_data_.size() + grid2_->grid_data_.size());
}

bool HGrid::isValidGridId(const int& grid_id) const {
  return grid_id >= 0 && grid_id < getGridCount();
}
