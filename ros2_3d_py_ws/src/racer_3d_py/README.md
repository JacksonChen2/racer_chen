# Python RACER 3-D for ROS 2 Humble and Isaac Sim

`racer_3d_py` is a pure-Python ROS 2/Isaac Sim implementation of the
three-dimensional RACER exploration pipeline. The planner, voxel map, HGrid,
multi-UAV allocation, safety layer, Crazyflie dynamics adapter and ROS nodes
are Python modules; no RACER C++ library is linked at build time or runtime.
It is packaged separately from `racer_3d`, so the workspace can be built and
...
The default Isaac installation path is `~/isaacsim`. Set `ISAAC_SIM_ROOT`
when Isaac is installed elsewhere.

## Build and test

```bash
cd RACER/ros2_3d_py_ws
...
```bash
RACER_3D_PY_DURATION=60 ROS_DOMAIN_ID=43 \
  src/racer_3d_py/scripts/run_mock_test.sh
```
...
```bash
RACER_3D_PY_DURATION=120 ROS_DOMAIN_ID=63 \
  src/racer_3d_py/scripts/run_isaac_test.sh \
  src/racer_3d_py/test_results/ISAAC_15X9X2_RESULT.json
```
...
```bash
RACER_3D_PY_SCENARIO=acceptance_20x50x3 \
RACER_3D_PY_DURATION=600 ROS_DOMAIN_ID=77 \
  src/racer_3d_py/scripts/run_isaac_test.sh \
  src/racer_3d_py/test_results/ISAAC_20X50X3_RESULT.json
```

`RACER_3D_PY_DURATION` is simulated mission time. A successful test stops as soon
...
```bash
RACER_3D_PY_SCENARIO=warehouse_simple \
RACER_3D_PY_DURATION=900 ROS_DOMAIN_ID=53 \
  src/racer_3d_py/scripts/run_isaac_test.sh \
  src/racer_3d_py/test_results/ISAAC_WAREHOUSE_SIMPLE_RESULT.json
```
...
```bash
RACER_3D_PY_SCENARIO=warehouse_simple \
RACER_3D_PY_DRONE_COUNT=5 RACER_3D_PY_DURATION=900 ROS_DOMAIN_ID=56 \
  src/racer_3d_py/scripts/run_isaac_test.sh \
  src/racer_3d_py/test_results/ISAAC_WAREHOUSE_SIMPLE_5UAV_RESULT.json
```
...
Cargo-populated Warehouse rack-zone test:

```bash
RACER_3D_PY_SCENARIO=warehouse_loaded \
RACER_3D_PY_DURATION=900 ROS_DOMAIN_ID=73 \
  src/racer_3d_py/scripts/run_isaac_test.sh \
  src/racer_3d_py/test_results/ISAAC_WAREHOUSE_LOADED_RESULT.json
```

This profile covers the complete generated cargo range in
`warehouse_loaded.usd` with a bounded 32.6 m × 19.0 m × 8.5 m rack-zone
volume and three PhysX-validated launch poses.

## Using a custom Isaac USD scene

The USD must contain collision-enabled geometry. Add a profile to
`racer_3d_py/scenario.py` so its map bounds, launch positions, HGrid scale and
...
source /opt/ros/humble/setup.bash
source install/setup.bash
ROS_DOMAIN_ID=64 ros2 launch racer_3d_py swarm_3d_py.launch.py \
...
env -u AMENT_PREFIX_PATH -u CMAKE_PREFIX_PATH -u COLCON_PREFIX_PATH \
  ...
  LD_LIBRARY_PATH="${ISAAC_SIM_ROOT:-${HOME}/isaacsim}/exts/isaacsim.ros2.bridge/humble/lib" \
  "${ISAAC_SIM_ROOT:-${HOME}/isaacsim}/python.sh" \
  src/racer_3d_py/isaac_sim/isaac_sim_racer_3d_py.py \
...
- distributed coordination: `/racer_3d_py/map_share`,
  `/racer_3d_py/swarm_state`, `/racer_3d_py/pairwise`;
- simulation/acceptance: `/racer_3d_py/sim_metrics`.
