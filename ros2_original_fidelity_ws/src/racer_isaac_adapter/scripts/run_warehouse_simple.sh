...
if [[ -n "${RACER_TRAJECTORY_OUTPUT:-}" ]]; then
  isaac_args+=(
    --trajectory-output "${RACER_TRAJECTORY_OUTPUT}"
    --trajectory-record-hz "${RACER_TRAJECTORY_RECORD_HZ:-60}"
  )
fi
...
wall_timeout_factor="${RACER_WALL_TIMEOUT_FACTOR:-20}"
wall_timeout_grace="${RACER_WALL_TIMEOUT_GRACE:-300}"
set +e
