...
OBSTACLE_CONTROL_CLEARANCE = max(0.30, VEHICLE_RADIUS + 0.236)
SWARM_CONTROL_DISTANCE = max(0.55, 2.0 * VEHICLE_RADIUS + 0.632)
SOURCE_MAX_SPEED = 0.85 if ARGS.vehicle_model == "racer_so3" else 0.35
...
def _low_level_safety_description() -> str:
    ...
    if SCENARIO.safety_min is not None and SCENARIO.safety_max is not None:
        return (
            "depth plus 360-degree safety-lidar and flight-volume "
            "stopping-distance barriers"
        )
...
class IsaacRacer3DBridge(Node):
    ...
    def _publish_clouds(self, stamp) -> None:
        for drone_id, (body, range_sensor) in enumerate(
                ...
                if raw_safety is not None:
                    ...
                    if len(safety_hits):
                        points = np.concatenate(
                            (points, safety_hits), axis=0
                        )
                        hit = np.concatenate(
                            (hit, np.ones(len(safety_hits), dtype=bool))
                        )
            else:
            ...
            if len(points) < 12:
                continue
            current_hits = np.asarray(points[hit], dtype=float)
