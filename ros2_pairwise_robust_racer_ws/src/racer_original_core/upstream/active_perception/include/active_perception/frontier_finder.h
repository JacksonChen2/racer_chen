...
public:
  ...
  // Build a transient reachable-only frontier view with one known-free connected-component
  // search. Returns true when a local, known-free expansion target is available as a fallback.
  bool getReachableViewpointsInfo(const Vector3d& cur_pos, Astar* path_finder,
      vector<Vector3d>& points, vector<double>& yaws, vector<Vector3d>& averages,
      vector<int>& frontier_ids, Vector3d& local_expansion_goal, double& local_expansion_yaw);
...
private:
  ...
  const vector<Viewpoint>& getPlanningViewpoints(const Frontier& frontier) const;
  const Viewpoint& getTopPlanningViewpoint(
  ...
  // Rebuilt for each task-selection round; persistent frontier data is never changed or
  // blacklisted by reachability failures.
  vector<vector<Viewpoint>> reachable_viewpoints_;
  bool reachability_filter_active_ = false;
