#!/usr/bin/env python3
"""Isaac Sim boundary for the source-faithful ROS 2/C++ C2-Explorer port.
...
# SIGUSR1 does not alter simulation or C2-Explorer state; it only writes Python
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "stop when the most complete shared C2-Explorer SDF map reaches this "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "drive render-only propeller Xforms from vehicle motor RPM; defaults "
    ...
    parser.add_argument(
        ...
        type=int,
        default=40000,
        help="display-only occupied-voxel cap; does not change the C2 map",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="racer_so3",
        help="PhysX vehicle plant used below the C++ C2 position command adapter",
    )
    parser.add_argument(
        "--vehicle-usd",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "write a compressed, simulation-time trajectory for decoupled "
            "Isaac Sim playback"
        ),
    )
    parser.add_argument(
        "--trajectory-record-hz",
        type=float,
        default=60.0,
        help=(
            "simulation-time sample rate for position, attitude, velocity, "
            "angular velocity, and four-motor RPM"
        ),
    )
...
if ARGS.trajectory_record_hz <= 0.0:
    raise SystemExit("--trajectory-record-hz must be positive")
if ARGS.trajectory_output is not None:
    ARGS.trajectory_output = ARGS.trajectory_output.expanduser().resolve()
    if ARGS.trajectory_output.suffix != ".npz":
        ARGS.trajectory_output = ARGS.trajectory_output.with_suffix(".npz")
...
class TrajectoryRecorder:
    """Sample authoritative PhysX state on the simulation clock."""

    def __init__(
        self,
        output: Path | None,
        sample_hz: float,
        bodies: Sequence[SingleRigidPrim],
    ) -> None:
        self.output = output
    ...
    def sample(
        self,
        elapsed: float,
        motor_rpms: Sequence[Sequence[float]],
        force: bool = False,
    ) -> None:
        if not self.enabled:
            return
        if not force and elapsed + 1.0e-12 < self.next_sample_time:
            return
        if self.times and abs(elapsed - self.times[-1]) < 1.0e-12:
            return
        ...
        self.linear_velocities.append(
            np.asarray(linear_velocities, dtype=np.float64)
        )
        self.angular_velocities.append(
            np.asarray(angular_velocities, dtype=np.float64)
        )
        self.motor_rpms.append(np.asarray(motor_rpms, dtype=np.float64))
        while self.next_sample_time <= elapsed + 1.0e-12:
            self.next_sample_time += self.sample_period

    def save(self, elapsed: float, stop_reason: str) -> dict | None:
        if not self.enabled:
            return None
        self.output.parent.mkdir(parents=True, exist_ok=True)
        metadata = {
            "schema": "racer_isaac_trajectory_v1",
            "algorithm": "original_racer_ros1_source_faithful_ros2_cpp",
            "scenario": ARGS.scenario,
            "scene_usd": str(ARGS.scene_usd) if ARGS.scene_usd else None,
            "vehicle_model": ARGS.vehicle_model,
            "vehicle_usd": str(ARGS.vehicle_usd),
            "drone_count": len(self.bodies),
            "sample_rate_hz": self.sample_hz,
            "physics_rate_hz": 1.0 / PHYSICS_DT,
            "recorded_elapsed_s": float(elapsed),
            "stop_reason": str(stop_reason),
            "start_positions": [list(point) for point in STARTS],
            "orientation_order": "wxyz",
            "motor_rpm_order": [0, 1, 2, 3],
            "motion_source": "Isaac Sim PhysX rotor force and torque",
        }
        ...
        with temporary.open("wb") as stream:
            np.savez_compressed(
                ...
                orientations_wxyz=np.asarray(
                    self.orientations, dtype=np.float64
                ),
                linear_velocities=np.asarray(
                    self.linear_velocities, dtype=np.float64
                ),
                angular_velocities=np.asarray(
                    self.angular_velocities, dtype=np.float64
                ),
                motor_rpm=np.asarray(self.motor_rpms, dtype=np.float64),
...
class IsaacRacer3DBridge(Node):
    def __init__(self, bodies, range_sensors, safety_sensors, contacts) -> None:
        ...
        self.cloud_frames = 0
        self.cloud_frames_per_drone = [0 for _ in bodies]
        self.cloud_none_per_drone = [0 for _ in bodies]
        self.cloud_small_per_drone = [0 for _ in bodies]
        ...
        self.metrics_publisher = self.create_publisher(
            String, "/c2_explorer/sim_metrics", qos
        )
    ...
    def _query_external_safety_points(
        ...
        published on ``/c2_explorer/sensor_points`` and therefore cannot change any
    ...
    def _publish_clouds(self, stamp) -> None:
        for drone_id, (body, range_sensor) in enumerate(
            ...
            if ARGS.vehicle_model == "racer_so3":
                ...
                safety_hits = np.empty((0, 3), dtype=float)
            if len(points) < 12:
                self.cloud_small_per_drone[drone_id] += 1
                continue
            ...
            self.cloud_frames += 1
            self.cloud_frames_per_drone[drone_id] += 1
    ...
    def publish_metrics(self) -> None:
        payload = {
            ...
            "point_cloud_frames_per_drone": self.cloud_frames_per_drone,
            "point_cloud_none_per_drone": self.cloud_none_per_drone,
            "point_cloud_small_per_drone": self.cloud_small_per_drone,
...
def main() -> None:
    ...
    bridge.cloud_frames_per_drone = [0 for _ in bodies]
    bridge.cloud_none_per_drone = [0 for _ in bodies]
    bridge.cloud_small_per_drone = [0 for _ in bodies]
    ...
    trajectory_recorder = TrajectoryRecorder(
        ...
        ARGS.trajectory_record_hz,
        bodies,
    )
    trajectory_recorder.sample(
        bridge.elapsed,
        bridge.motor_rpms,
        force=True,
    )
    ...
    try:
        ...
        trajectory_recorder.save(bridge.elapsed, stop_reason)
        bridge.publish_metrics()
            ...
            + json.dumps(
                {
                    ...
                    "point_cloud_frames_per_drone": (
                        bridge.cloud_frames_per_drone
                    ),
                    "point_cloud_none_per_drone": (
                        bridge.cloud_none_per_drone
                    ),
                    "point_cloud_small_per_drone": (
                        bridge.cloud_small_per_drone
                    ),
