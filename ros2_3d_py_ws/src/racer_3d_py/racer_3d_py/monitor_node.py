...
class Racer3DMonitor(Node):
    def __init__(self) -> None:
        super().__init__("racer_3d_py_monitor")
        ...
        self.declare_parameter("result_file", "/tmp/racer_3d_py_result.json")
        ...
        self.completion_publisher = self.create_publisher(
            String, "/racer_3d_py/mission_complete", qos
        )
        self.create_subscription(
            String, "/racer_3d_py/map_share", self._map_callback, qos
        )
        self.create_subscription(
            String, "/racer_3d_py/sim_metrics", self._metrics_callback, qos
        )
    ...
    def _finish(self, elapsed: float, quality: dict) -> None:
        ...
        print("RACER_3D_PY_ACCEPTANCE " + json.dumps(result, sort_keys=True),
              flush=True)
        self.finished = True
        rclpy.shutdown()
