...
int C2ExplorationManager::planExploreMotion(
  ...
  if (grid_ids.empty()) {
    ...
    // A frontier may disappear between cost-matrix construction and viewpoint
    // refinement as a new Isaac point-cloud frame is fused.  The ROS1 code
    // assumes the refined list is non-empty; preserve its selected top
    // viewpoint as the fallback instead of indexing an empty transient list.
    if (ed_->n_points_.empty() || n_yaws.empty() || ed_->n_points_[0].empty() ||
        n_yaws[0].empty()) {
      ROS_WARN("Refined viewpoint disappeared; use the original top viewpoint");
      ...
      const int candidate_num = std::min(ed_->n_points_[0].size(), n_yaws[0].size());
...
int C2ExplorationManager::planTrajToView(const Vector3d& pos, const Vector3d& vel,
  ...
  // A start and goal that collapse to the same point produce a one-point
  // shortened path.  The upstream polynomial generator requires at least one
  // segment; use the FSM's existing planning-failure retry for this no-op goal
  // instead of indexing its zero-sized matrices.
  if (ed_->path_next_goal_.size() < 2) {
    ROS_WARN("Degenerate one-point path; retry exploration planning");
    return FAIL;
  }
