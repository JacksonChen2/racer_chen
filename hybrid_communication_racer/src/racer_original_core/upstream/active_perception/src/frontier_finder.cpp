...
void FrontierFinder::searchFrontiers() {
  ...
  reachable_viewpoints_.clear();
  reachability_filter_active_ = false;
...
bool FrontierFinder::getReachableViewpointsInfo(const Vector3d& cur_pos, Astar* path_finder,
    vector<Eigen::Vector3d>& points, vector<double>& yaws,
    vector<Eigen::Vector3d>& averages, vector<int>& frontier_ids,
    Vector3d& local_expansion_goal, double& local_expansion_yaw) {
  points.clear();
  yaws.clear();
  averages.clear();
  frontier_ids.clear();
  reachable_viewpoints_.assign(frontiers_.size(), {});
  reachability_filter_active_ = true;

  struct CandidateRef {
    int frontier_id;
    const Viewpoint* viewpoint;
  };
  vector<CandidateRef> candidates;
  ...
  for (const auto& frontier : frontiers_) {
    for (const auto& view : frontier.viewpoints_) {
      ++all_candidate_count;
      if (!edt_env_->sdf_map_->isInBox(view.pos_) ||
          edt_env_->sdf_map_->getInflateOccupancy(view.pos_) == 1 ||
          edt_env_->sdf_map_->getOccupancy(view.pos_) == SDFMap::UNKNOWN)
        continue;
      candidates.push_back({ frontier.id_, &view });
      candidate_positions.push_back(view.pos_);
    }
  }

  vector<bool> reachable;
  const bool have_local_expansion = path_finder->searchKnownFreeComponent(
      cur_pos, candidate_positions, reachable, local_expansion_goal);
  int reachable_count = 0;
  for (std::size_t i = 0; i < candidates.size(); ++i) {
    if (!reachable[i]) continue;
    reachable_viewpoints_[candidates[i].frontier_id].push_back(*candidates[i].viewpoint);
    ++reachable_count;
  }

  for (const auto& frontier : frontiers_) {
    if (frontier.id_ < 0 || frontier.id_ >= static_cast<int>(reachable_viewpoints_.size()) ||
        reachable_viewpoints_[frontier.id_].empty())
      continue;
    const auto& selected = getTopPlanningViewpoint(frontier, cur_pos);
    points.push_back(selected.pos_);
    yaws.push_back(selected.yaw_);
    averages.push_back(frontier.average_);
    frontier_ids.push_back(frontier.id_);
  }

  local_expansion_yaw = 0.0;
  if (have_local_expansion && !frontiers_.empty()) {
    const Frontier* nearest = &frontiers_.front();
    double nearest_distance = (nearest->average_ - local_expansion_goal).squaredNorm();
    for (const auto& frontier : frontiers_) {
      const double distance = (frontier.average_ - local_expansion_goal).squaredNorm();
      if (distance < nearest_distance) {
        nearest = &frontier;
        nearest_distance = distance;
      }
    }
    const Vector3d direction = nearest->average_ - local_expansion_goal;
    local_expansion_yaw = atan2(direction.y(), direction.x());
  }

  ROS_INFO("Connectivity reachability: %d/%d viewpoints reachable, %zu/%zu frontiers retained, "
           "local expansion=%d.", reachable_count, all_candidate_count, frontier_ids.size(),
      frontiers_.size(), int(have_local_expansion));
  return have_local_expansion;
}

const vector<Viewpoint>& FrontierFinder::getPlanningViewpoints(const Frontier& frontier) const {
  if (reachability_filter_active_ && frontier.id_ >= 0 &&
      frontier.id_ < static_cast<int>(reachable_viewpoints_.size()) &&
      !reachable_viewpoints_[frontier.id_].empty())
    return reachable_viewpoints_[frontier.id_];
  return frontier.viewpoints_;
}

const Viewpoint& FrontierFinder::getTopPlanningViewpoint(
    const Frontier& frontier, const Vector3d& cur_pos) const {
  const auto& viewpoints = getPlanningViewpoints(frontier);
  for (const auto& view : viewpoints) {
    if ((view.pos_ - cur_pos).norm() >= min_candidate_dist_) return view;
  }
  return viewpoints.front();
}
...
void FrontierFinder::getViewpointsInfo(const Vector3d& cur_pos, const vector<int>& ids,
  ...
  for (auto id : ids) {
    ...
    for (auto frontier : frontiers_) {
      if (frontier.id_ == id) {
        const vector<Viewpoint>* planning_viewpoints = &frontier.viewpoints_;
        if (reachability_filter_active_ && id >= 0 &&
            id < static_cast<int>(reachable_viewpoints_.size()))
          planning_viewpoints = &reachable_viewpoints_[id];

        // Get several top viewpoints that are far enough
        vector<Eigen::Vector3d> pts;
        vector<double> ys;
        if (planning_viewpoints->empty()) {
          points.push_back(pts);
          yaws.push_back(ys);
          break;
        }
        int visib_thresh = planning_viewpoints->front().visib_num_ * max_decay;
        for (auto view : *planning_viewpoints) {
...
void FrontierFinder::getPathForTour(
  ...
  const auto& first_view = getTopPlanningViewpoint(*frontier_indexer[frontier_ids[0]], pos);
  ViewNode::searchPath(pos, first_view.pos_, segment);
...
void FrontierFinder::getFullCostMatrix(const Vector3d& cur_pos, const Vector3d& cur_vel,
  ...
  for (auto ftr : frontiers_) {
    // std::cout << "(0, " << j << ")"
    // << ", ";
    Viewpoint vj = getTopPlanningViewpoint(ftr, cur_pos);
...
void FrontierFinder::getSwarmCostMatrix(const vector<Vector3d>& positions,
  ...
  for (int i = 0; i < drone_num; ++i) {
    int j = 0;
    for (auto ftr : frontiers_) {
      Viewpoint vj = getTopPlanningViewpoint(ftr, positions[i]);
      vector<Vector3d> path;
...
void FrontierFinder::getSwarmCostMatrix(const vector<Vector3d>& positions,
  ...
  // Build only the reachable subset. Calling the full-matrix overload here would still run path
  // costs from the UAV to every filtered-out frontier and defeat the pre-selection filter.
  vector<list<Frontier>::iterator> frontier_indexer;
  frontier_indexer.reserve(frontiers_.size());
  ...
  for (int i = 0; i < drone_num; ++i) {
    for (int j = 0; j < ftr_num; ++j) {
      const auto& view =
          getTopPlanningViewpoint(*frontier_indexer[ftr_ids[j]], positions[i]);
      vector<Vector3d> path;
      mat(1 + i, 1 + drone_num + j) = ViewNode::computeCost(positions[i], view.pos_,
          yaws[i], view.yaw_, velocities[i], 0.0, path);
      mat(1 + drone_num + j, 1 + i) = 0;
    }
  }
  // Costs between frontiers
  for (int i = 0; i < ftr_num; ++i) {
    for (int j = 0; j < ftr_num; ++j) {
      auto cost = frontier_indexer[ftr_ids[i]]->costs_.begin();
      std::advance(cost, ftr_ids[j]);
  ...
  if (!grid_pos.empty()) {
    ...
    for (int i = 0; i < ftr_num; ++i) {
      const auto& view =
          getTopPlanningViewpoint(*frontier_indexer[ftr_ids[i]], positions[0]);
      vector<Eigen::Vector3d> path;
      double cost = ViewNode::computeCost(
          next_grid, view.pos_, 0, 0, Eigen::Vector3d(0, 0, 0), 0, path);
