...
from racer_3d_interfaces.msg import LinkQuality, LinkQualityArray
...
class SionnaChannelNode(Node):
    def __init__(self) -> None:
        super().__init__("racer_3d_sionna_channel")
        ...
        self.link_publisher = self.create_publisher(
            LinkQualityArray, "/racer_3d/link_quality", qos
        )
        self.status_publisher = self.create_publisher(
            String, "/racer_3d/channel_status", qos
        )
    ...
    def _initialize_sionna(self) -> None:
        ...
        try:
            ...
            for drone_id in range(self.drone_count):
                tx = Transmitter(
                    name=f"uav_tx_{drone_id}",
                    position=[0.0, 0.0, -1000.0],
                    power_dbm=self.tx_power_dbm,
                )
                rx = Receiver(
                    name=f"uav_rx_{drone_id}", position=[0.0, 0.0, -1000.0]
                )
                self.scene.add(tx)
                self.scene.add(rx)
            ...
            self.get_logger().info(f"loaded Sionna RT scene {self.scene_xml}")
        except Exception as exception:  # noqa: BLE001
    ...
    def _request_solve(self) -> None:
        if not self.sionna_ready:
            return
        with self.pose_lock:
            if len(self.poses) != self.drone_count:
                return
        ...
        if moved or time.monotonic() - getattr(self, "last_solve_wall", -math.inf) >= 1.0 / self.solver_rate_hz:
    ...
    def _extract_paths(self, paths, snapshot: Dict[int, dict]) -> Dict[Tuple[int, int], dict]:
        ...
        if gain.shape != (self.drone_count, self.drone_count):
        ...
        output = {}
        for receiver in range(self.drone_count):
            for sender in range(self.drone_count):
                if sender == receiver:
                    continue
    ...
    def _metric(self, sender: int, receiver: int, positions, stamp: float) -> dict:
        ...
        rss_dbm = self.tx_power_dbm + gain_db
        return {
            "distance": distance,
            "gain_db": gain_db,
            "rss_dbm": rss_dbm,
