...
def _launch_nodes(context):
    ...
    task_metric_sample_period_s = float(
        LaunchConfiguration("task_metric_sample_period_s").perform(context)
    )
    ground_truth_occupied_voxels_path = LaunchConfiguration(
        "ground_truth_occupied_voxels_path"
    ).perform(context)
    task_metric_output_path = LaunchConfiguration(
        ...
        Node(
            ...
            parameters=[
                communication_config,
                {
                    ...
                    "task_metric_sample_period_s": task_metric_sample_period_s,
                    "ground_truth_occupied_voxels_path": (
                        ground_truth_occupied_voxels_path
                    ),
                    "task_metric_output_path": task_metric_output_path,
                },
            ],
        )
    )
        ...
        [
            ...
            DeclareLaunchArgument(
                ...
                default_value=os.environ.get(
                    "RACER_TASK_METRIC_OBSERVER_MODE", "off"
                ),
            ),
            DeclareLaunchArgument(
                "task_metric_sample_period_s",
                default_value=os.environ.get(
                    "RACER_TASK_METRIC_SAMPLE_PERIOD_S", "0.1"
                ),
            ),
            DeclareLaunchArgument(
                "ground_truth_occupied_voxels_path",
                default_value=os.environ.get(
                    "RACER_GROUND_TRUTH_OCCUPIED_VOXELS_PATH", ""
                ),
            ),
            DeclareLaunchArgument(
                "task_metric_output_path",
                default_value=os.environ.get(
                    "RACER_TASK_METRIC_OUTPUT_PATH", ""
                ),
            ),
            DeclareLaunchArgument(
