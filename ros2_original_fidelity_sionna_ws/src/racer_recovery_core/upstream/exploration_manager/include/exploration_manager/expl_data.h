...
struct FSMParam {
  ...
  // Bounded recovery extension. These parameters are consumed only by the
  // recovery fork and never alter the source-faithful racer_original_core.
  bool recovery_enabled_;
  bool recovery_ap_enabled_;
  double recovery_retry_backoff_s_;
  double recovery_no_path_duration_s_;
  int recovery_no_path_count_;
  double recovery_min_progress_m_;
  double recovery_escape_distance_m_;
  double recovery_escape_clearance_m_;
  double recovery_ap_wait_timeout_s_;
  double recovery_status_period_s_;
