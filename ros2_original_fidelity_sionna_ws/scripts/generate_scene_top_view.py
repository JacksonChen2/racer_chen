...
def draw_ap(axis, ap_position) -> None:
    axis.scatter(
    ...
    handles.extend(
        [
            Line2D([0], [0], marker="D", linestyle="", markersize=7,
                   markerfacecolor="#e31a1c", markeredgecolor="white", label="industrial AP"),
...
def main() -> None:
    ...
    draw_ap(axis, args.ap_position)
    axis.set_title(
        "Warehouse Full with Industrial AP — orthographic scene top view\n"
    ...
    draw_ap(clean_axis, args.ap_position)
    ...
    for axis, height_entry in zip(axes.flat[1:], selection["heights"]):
        ...
        axis.set_title(f"Actual P01–P25 UAV TX positions | z={height_entry['height_m']:g} m")
    figure.suptitle(
        "Warehouse Full with Industrial AP — scene/radio-map correspondence\n"
        "AP is shown as scene geometry/reference only; it is inactive in the UAV-to-UAV maps",
        fontsize=15,
    )
