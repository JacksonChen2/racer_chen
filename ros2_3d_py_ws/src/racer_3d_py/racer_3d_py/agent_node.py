...
class Racer3DAgent(Node):
    ...
    def __init__(self) -> None:
        super().__init__("racer_3d_py_agent")
        ...
        self.map_share_publisher = self.create_publisher(
            String, "/racer_3d_py/map_share", reliable
        )
        self.state_publisher = self.create_publisher(
            String, "/racer_3d_py/swarm_state", reliable
        )
        self.allocation_publisher = self.create_publisher(
            String, "/racer_3d_py/pairwise", reliable
        )
        ...
        self.create_subscription(
            String, "/racer_3d_py/map_share", self._map_callback, reliable
        )
        self.create_subscription(
            String, "/racer_3d_py/swarm_state", self._state_callback, reliable
        )
        self.create_subscription(
            String, "/racer_3d_py/pairwise", self._allocation_callback, reliable
        )
...
def main(args=None) -> None:
    ...
    try:
        ...
        # A launch-wide SIGINT can invalidate the context while a timer
        # callback is completing its final publish.  Suppress only that
        # normal shutdown race; a live-context middleware error remains fatal.
        if rclpy.ok():
            raise
    finally:
        node.destroy_node()
