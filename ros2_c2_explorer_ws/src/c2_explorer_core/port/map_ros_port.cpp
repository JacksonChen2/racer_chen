...
void MapROS::init() {
  ...
  cloud_sub_ = node_.subscribe("/c2_explorer/sensor_points", 5,
...
void MapROS::cloudCallback(const sensor_msgs::PointCloud2ConstPtr &message) {
  ...
  // Match the ROS1 cloudOdomCallback ordering exactly: occupancy inflation is
  // completed in the point-cloud callback before any later FSM/frontier timer
  // can observe the new map; only the more expensive ESDF pass is deferred to
  // its 50 ms timer.
  if (local_updated_) {
...
void MapROS::updateESDFCallback(const ros::TimerEvent &) {
  if (!map_ || !esdf_need_update_) return;
  map_->updateESDF3d();
  esdf_need_update_ = false;
}
...
void MapROS::coverageCallback(const ros::TimerEvent &) {
  ...
  ROS_INFO("C2_MAP_COVERAGE known=%llu total=%llu ratio=%.6f",
