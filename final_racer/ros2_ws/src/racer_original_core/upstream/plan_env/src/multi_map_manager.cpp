...
void MultiMapManager::init() {
  ...
  node_.param("oracle_shared_map/chunk_period", chunk_period_s_, 0.02);
  node_.param("oracle_shared_map/stamp_throttle", stamp_throttle_s_, 0.05);
  // The oracle transport is lossless and every completed local chunk is
  // proactively fanned out below. Running the original stamp/missing-chunk
  // anti-entropy loop as well only regenerates already delivered ChunkData.
  // Keep that protocol compiled into the source-faithful target, but disable
  // it in this separate oracle algorithm.
  chunk_timer_ = node_.createTimer(
      ros::Duration(chunk_period_s_), &MultiMapManager::chunkTimerCallback, this);
  ROS_WARN("RACER_ORACLE_SHARED_MAP enabled chunk_size=%d process=%.3fs anti_entropy=disabled",
      chunk_size_, chunk_period_s_);
#else
...
void MultiMapManager::updateMapChunk(const vector<uint32_t>& adrs) {
  ...
  if (adr_buffer_.size() >= chunk_size_) {
    const int first_new_chunk = multi_map_chunks_[drone_id_ - 1].chunks_.size() + 1;
...
void MultiMapManager::stampMsgCallback(const plan_env::ChunkStampsConstPtr& msg) {
  // from_drone_id == 0 is reserved for a local BS upload request.  The
  // communication Proxy builds idx_lists from at most 32 exact owner/index
  // keys selected by the RL action.  Reuse the original ChunkStamps ->
  // sendChunks() path so a payload that the Proxy has never observed is
  // materialized from this UAV's real multi_map_chunks_ store as an ordinary
  // ChunkData message.  to_drone_id == 0 lets the Proxy recognize the reply
  // and put it through its normal UAV->BS radio queue.
  if (msg->from_drone_id == 0) {
    constexpr int kMaxBsRequestedChunks = 32;
    int remaining = kMaxBsRequestedChunks;
    const int owner_count = min(
        static_cast<int>(msg->idx_lists.size()),
        static_cast<int>(multi_map_chunks_.size()));
    for (int owner = 0; owner < owner_count && remaining > 0; ++owner) {
      const auto& requested = msg->idx_lists[owner].ids;
      const auto& available = multi_map_chunks_[owner].idx_list_;
      vector<int> selected;
      // Intersect the requested ranges with the chunks this UAV really owns.
      // This also prevents stale or malformed requests from indexing an empty
      // placeholder in chunks_.
      for (int request = 0;
           request + 1 < requested.size() && remaining > 0; request += 2) {
        if (requested[request] < 1 ||
            requested[request + 1] < requested[request]) {
          continue;
        }
        for (int own = 0;
             own + 1 < available.size() && remaining > 0; own += 2) {
          int first, last;
          if (!findIntersect(requested[request], requested[request + 1],
                  available[own], available[own + 1], first, last)) {
            continue;
          }
          last = min(last, first + remaining - 1);
          selected.push_back(first);
          selected.push_back(last);
          remaining -= last - first + 1;
        }
      }
      if (!selected.empty()) sendChunks(owner + 1, 0, selected);
    }
    return;
  }
  ...
  if (msg->from_drone_id < 1 || msg->from_drone_id > map_num_ ||
      msg->idx_lists.size() < multi_map_chunks_.size()) {
    return;
  }
...
#ifdef RACER_ORACLE_VARIANT
  if (msg->time - last_chunk_stamp_time_[msg->from_drone_id - 1] < stamp_throttle_s_) return;
#else
