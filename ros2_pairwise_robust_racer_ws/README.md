...
```bash
cd /path/to/racer_chen/ros2_pairwise_robust_racer_ws
./build_nlopt.sh
./setup_sionna_env.sh
...
当前 10 架无人机、五区域、固定 MCS 14 的 Pairwise Robust RACER 入口为：

```bash
./run_warehouse_full_10uav_five_sites_sionna_300s.sh
```

该入口只依赖仓库内的 `config/warehouse_full_10uav_five_sites_layout.json`、
`warehouse_scenes/isaac/warehouse_full_with_industrial_ap.usda` 和对应的
