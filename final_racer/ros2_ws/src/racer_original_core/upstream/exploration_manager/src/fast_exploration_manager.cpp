...
void FastExplorationManager::initialize(ros::NodeHandle& nh) {
  ...
  for (auto& state : ed_->swarm_state_) {
    ...
    state.assignment_epoch_ = 0;
    state.commitment_until_ = 0.0;
    state.requesting_work_ = false;
  }
  ed_->last_grid_ids_ = {};
...
int FastExplorationManager::planExploreMotion(
  ...
  // Build a transient reachable-only view of all frontier candidates. A single known-free
  // connected-component search applies the same map constraints as A*: unknown, inflated
  // obstacles and positions outside the planning boundary are rejected. The cache is rebuilt on
  // every retry, so an unreachable candidate can become eligible after the map grows.
  Vector3d local_expansion_goal;
  double local_expansion_yaw = yaw[0];
  const bool have_local_expansion = frontier_finder_->getReachableViewpointsInfo(pos,
      planner_manager_->path_finder_.get(), ed_->points_, ed_->yaws_, ed_->averages_,
      reachable_frontier_ids_, local_expansion_goal, local_expansion_yaw);
  ed_->views_.clear();
  for (int i = 0; i < ed_->points_.size(); ++i)
    ed_->views_.push_back(
        ed_->points_[i] + 2.0 * Vector3d(cos(ed_->yaws_[i]), sin(ed_->yaws_[i]), 0));

  if (reachable_frontier_ids_.empty()) {
    if (have_local_expansion && (local_expansion_goal - pos).norm() > 0.3) {
      ROS_WARN_THROTTLE(1.0,
          "No frontier viewpoint is currently reachable; expanding the local known-free "
          "component before retrying frontier selection.");
      ed_->next_pos_ = local_expansion_goal;
      ed_->next_yaw_ = local_expansion_yaw;
      ed_->refined_points_ = { local_expansion_goal };
      ed_->refined_views_ = { local_expansion_goal +
        2.0 * Vector3d(cos(local_expansion_yaw), sin(local_expansion_yaw), 0) };
      return planTrajToView(pos, vel, acc, yaw, local_expansion_goal, local_expansion_yaw);
    }
    ROS_WARN_THROTTLE(1.0,
        "All candidate viewpoints are unreachable through known free space; retry planning.");
    return FAIL;
  }
  ...
  if (grid_ids.empty()) {
    if (!allow_unassigned_frontier_fallback_) return NO_GRID;

    // A global rebalance is intentionally infrequent.  Do useful local work
    // in the meantime without changing grid ownership: select the closest
    // reachable frontier from this UAV's current map.
    ROS_WARN_THROTTLE(1.0,
        "No assigned grid; use nearest reachable local frontier while awaiting global assignment.");
    ...
    if (min_cost_id < 0) return NO_GRID;
    next_pos = ed_->points_[min_cost_id];
    next_yaw = ed_->yaws_[min_cost_id];
    ed_->frontier_tour_ = { pos, next_pos };
    ed_->refined_ids_ = { min_cost_id };
    ...
    if (ep_->refine_local_) {
      ...
      const vector<int> original_ids = { reachable_frontier_ids_[frontier_ids[0]] };
      frontier_finder_->getViewpointsInfo(
          pos, original_ids, ep_->top_view_num_, ep_->max_decay_, ed_->n_points_, n_yaws);
    ...
    ed_->n_points_.clear();
    vector<vector<double>> n_yaws;
    vector<int> original_refined_ids;
    original_refined_ids.reserve(ed_->refined_ids_.size());
    for (const auto id : ed_->refined_ids_)
      original_refined_ids.push_back(reachable_frontier_ids_[id]);
    frontier_finder_->getViewpointsInfo(
        pos, original_refined_ids, ep_->top_view_num_, ep_->max_decay_, ed_->n_points_, n_yaws);
...
void FastExplorationManager::updateGlobalGridPool(
    const Eigen::Vector3d& pos, vector<int>& grid_ids) {
  updateFrontierStruct(pos);
  hgrid_->inputFrontiers(ed_->averages_);

  // updateGridData also refreshes unknown counts for map regions changed by
  // shared chunks. Use a copy so global coordination cannot rewrite the
  // currently committed route merely by observing the grid structure.
  vector<int> scratch = ed_->swarm_state_[ep_->drone_id_ - 1].grid_ids_;
  vector<int> first_ids, second_ids;
  hgrid_->updateGridData(
      ep_->drone_id_, scratch, false, ed_->last_grid_ids_, first_ids, second_ids);
  hgrid_->getActiveGrids(grid_ids);
  std::sort(grid_ids.begin(), grid_ids.end());
  grid_ids.erase(std::unique(grid_ids.begin(), grid_ids.end()), grid_ids.end());
}

double FastExplorationManager::estimateGridTravelCost(
    const Eigen::Vector3d& pos, int grid_id) {
  if (!hgrid_->isValidGridId(grid_id))
    return std::numeric_limits<double>::infinity();
  return hgrid_->getCostDroneToGrid(pos, grid_id, {});
}
...
void FastExplorationManager::findTourOfFrontier(const Vector3d& cur_pos, const Vector3d& cur_vel,
  ...
  vector<int> original_ftr_ids;
  original_ftr_ids.reserve(ftr_ids.size());
  for (const auto id : ftr_ids) {
    if (id < 0 || id >= static_cast<int>(reachable_frontier_ids_.size())) {
      ROS_ERROR("Invalid reachable frontier index %d.", id);
      indices.clear();
      return;
    }
    original_ftr_ids.push_back(reachable_frontier_ids_[id]);
  }

  // frontier_finder_->getSwarmCostMatrix(positions, velocities, yaws, mat);
  Eigen::MatrixXd mat;
  frontier_finder_->getSwarmCostMatrix(
      positions, velocities, yaws, original_ftr_ids, grid_pos, mat);
  const int dimension = mat.rows();
  ...
  file << "MTSP_OBJECTIVE = MINSUM\n";
  file << "SEED = " << ep_->random_seed_ << "\n";
  file << "MTSP_MIN_SIZE = " << to_string(min(int(ftr_ids.size()) / drone_num, 4)) << "\n";
  file << "MTSP_MAX_SIZE = "
       << to_string(max(1, int(ed_->frontiers_.size()) / max(1, drone_num - 1))) << "\n";
  file << "RUNS = 1\n";
  file << "TRACE_LEVEL = 0\n";
  ...
  original_indices.reserve(indices.size());
  for (const auto id : indices) original_indices.push_back(reachable_frontier_ids_[id]);
  frontier_finder_->getPathForTour(cur_pos, original_indices, ed_->frontier_tour_);
  if (!grid_pos.empty()) {
