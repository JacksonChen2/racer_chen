...
class CommunicationProxy final : public rclcpp::Node {
 public:
  CommunicationProxy()
      : Node("racer_sionna_communication_proxy"),
        ...
        map_perfect_delivery_(declare_parameter<bool>("map_perfect_delivery", false)),
 ...
 private:
  ...
  void onTransmit(int sender, std::size_t topic_index,
    ...
    if (map_perfect_delivery_ &&
        (topic_key == "chunk_data" || topic_key == "chunk_stamps")) {
      ++map_perfect_messages_;
  ...
  void publishStatistics() {
    ...
    json << "{\"network_topology\":\"" << topology_
         ...
         << ",\"map_perfect_delivery_enabled\":"
         << (map_perfect_delivery_ ? "true" : "false")
         << ",\"map_perfect_messages\":" << map_perfect_messages_
  ...
  bool map_perfect_delivery_{false};
  std::uint64_t map_perfect_messages_{};
