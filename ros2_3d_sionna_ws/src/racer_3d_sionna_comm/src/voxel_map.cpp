...
void VoxelMap::mergeSparse(const std::vector<std::uint32_t> &indices,
                           const std::vector<std::int8_t> &values) {
  if (indices.size() != values.size()) {
    throw std::invalid_argument("sparse map indices/states size mismatch");
  }
  bool changed = false;
  for (std::size_t offset = 0; offset < indices.size(); ++offset) {
    const std::size_t index = indices[offset];
    if (index >= voxelCount() || values[offset] < 0) {
      continue;
    }
    observations_[index] = std::max<std::uint16_t>(observations_[index], 1U);
    if (values[offset] == FREE && log_odds_[index] < 2) {
      log_odds_[index] = std::min<std::int16_t>(log_odds_[index], -8);
      changed = true;
    } else if (values[offset] == OCCUPIED) {
      log_odds_[index] = std::max<std::int16_t>(log_odds_[index], 10);
      changed = true;
    }
  }
