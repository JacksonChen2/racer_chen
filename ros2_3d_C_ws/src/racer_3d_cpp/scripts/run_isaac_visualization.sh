...
if [[ -z "${RACER_3D_VISUALIZATION_MAX_MAP_POINTS:-}" ]]; then
  if (( RACER_3D_DRONE_COUNT >= 5 )); then
    export RACER_3D_VISUALIZATION_MAX_MAP_POINTS=12000
  else
    export RACER_3D_VISUALIZATION_MAX_MAP_POINTS=40000
  fi
fi
export RACER_3D_INTERACTIVE_RENDER_HZ="${RACER_3D_INTERACTIVE_RENDER_HZ:-30}"
...
if [[ -z "${RACER_3D_PROPELLER_VISUAL_HZ:-}" ]]; then
  if (( RACER_3D_DRONE_COUNT >= 5 )); then
    export RACER_3D_PROPELLER_VISUAL_HZ=20
  else
    export RACER_3D_PROPELLER_VISUAL_HZ=60
  fi
fi
...
printf 'Starting RACER Isaac visualization: scenario=%s drones=%s duration=%ss ROS_DOMAIN_ID=%s vehicle=%s propeller_visual_hz=%s interactive_render_hz=%s\n' \
  "${RACER_3D_SCENARIO}" "${RACER_3D_DRONE_COUNT}" \
  "${RACER_3D_DURATION}" "${ROS_DOMAIN_ID}" \
  "${RACER_SO3_VEHICLE_USD}" "${RACER_3D_PROPELLER_VISUAL_HZ}" \
  "${RACER_3D_INTERACTIVE_RENDER_HZ}"
printf 'Legend: occupied map=cyan; drone trails/plans=red, green, yellow, purple, orange\n'
