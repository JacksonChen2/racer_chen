...
import racer_3d_py.planning as planning_module
from racer_3d_py.planning import (
    astar3d,
    minimum_time_bspline_trajectory,
    shorten_path3d,
)
from racer_3d_py.scenario import (
    DRONE_RADIUS,
    get_scenario,
    obstacle_clearance,
)
from racer_3d_py.voxel_map import FREE, OCCUPIED, VoxelMap
...
def test_warehouse_loaded_profile_covers_generated_cargo_zone():
    scenario = get_scenario("warehouse_loaded")
    assert scenario.map_size == (32.6, 19.0, 8.5)
    assert scenario.truth_mode == "observed_volume"
    assert scenario.obstacles == ()
    ...
    # Extrema from warehouse_loaded_cargo_report.json.
    generated_min = (-25.3773, 9.4816, 1.5957)
    generated_max = (4.4511, 24.2265, 6.4522)
    assert all(
        lower < value
        for lower, value in zip(scenario.map_min, generated_min)
    )
    assert all(
        value < upper
        for value, upper in zip(generated_max, scenario.map_max)
    )
    assert all(
        all(
            lower < value < upper
