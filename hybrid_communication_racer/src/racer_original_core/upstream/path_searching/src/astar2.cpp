...
void Astar::init(ros::NodeHandle& nh, const EDTEnvironment::Ptr& env) {
  ...
  nh.param("reachability/connectivity_max_nodes", connectivity_max_nodes_, 300000);
  nh.param("reachability/local_expansion_min_dist", local_expansion_min_dist_, 0.8);
  nh.param("reachability/local_expansion_max_dist", local_expansion_max_dist_, 3.0);
...
bool Astar::searchKnownFreeComponent(const Eigen::Vector3d& start_pt,
    const std::vector<Eigen::Vector3d>& goals, std::vector<bool>& reachable,
    Eigen::Vector3d& local_expansion_goal) {
  struct ComponentNode {
    Eigen::Vector3i index;
    Eigen::Vector3d position;
  };

  reachable.assign(goals.size(), false);
  local_expansion_goal = start_pt;
  if (goals.empty()) return false;

  std::unordered_map<Eigen::Vector3i, std::vector<std::size_t>,
      matrix_hash<Eigen::Vector3i>> goal_bins;
  for (std::size_t i = 0; i < goals.size(); ++i) {
    Eigen::Vector3i goal_index;
    posToIndex(goals[i], goal_index);
    for (int dx = -1; dx <= 1; ++dx)
      for (int dy = -1; dy <= 1; ++dy)
        for (int dz = -1; dz <= 1; ++dz)
          goal_bins[goal_index + Eigen::Vector3i(dx, dy, dz)].push_back(i);
  }

  Eigen::Vector3i start_index;
  posToIndex(start_pt, start_index);
  std::queue<ComponentNode> frontier;
  std::unordered_map<Eigen::Vector3i, int, matrix_hash<Eigen::Vector3i>> visited;
  frontier.push({ start_index, start_pt });
  visited.emplace(start_index, 1);

  std::size_t reached_count = 0;
  double best_expansion_distance = -1.0;
  const int node_limit = std::max(1, std::min(connectivity_max_nodes_, allocate_num_));
  bool node_limit_hit = false;

  while (!frontier.empty()) {
    const ComponentNode current = frontier.front();
    frontier.pop();

    const auto goal_it = goal_bins.find(current.index);
    if (goal_it != goal_bins.end()) {
      for (const std::size_t goal_id : goal_it->second) {
        if (!reachable[goal_id]) {
          reachable[goal_id] = true;
          ++reached_count;
        }
      }
      if (reached_count == goals.size()) break;
    }

    const double distance_from_start = (current.position - start_pt).norm();
    if (distance_from_start >= local_expansion_min_dist_ &&
        distance_from_start <= local_expansion_max_dist_ &&
        edt_env_->sdf_map_->getInflateOccupancy(current.position) != 1 &&
        edt_env_->sdf_map_->getOccupancy(current.position) != SDFMap::UNKNOWN) {
      bool borders_unknown = false;
      for (int axis = 0; axis < 3 && !borders_unknown; ++axis) {
        for (int sign : { -1, 1 }) {
          Eigen::Vector3d probe = current.position;
          probe[axis] += sign * resolution_;
          if (edt_env_->sdf_map_->isInBox(probe) &&
              edt_env_->sdf_map_->getOccupancy(probe) == SDFMap::UNKNOWN) {
            borders_unknown = true;
            break;
          }
        }
      }
      if (borders_unknown && distance_from_start > best_expansion_distance) {
        best_expansion_distance = distance_from_start;
        local_expansion_goal = current.position;
      }
    }
    ...
    for (double dx = -resolution_; dx <= resolution_ + 1e-3; dx += resolution_)
      for (double dy = -resolution_; dy <= resolution_ + 1e-3; dy += resolution_)
        for (double dz = -resolution_; dz <= resolution_ + 1e-3; dz += resolution_) {
          Eigen::Vector3d step(dx, dy, dz);
          if (step.norm() < 1e-3) continue;
          const Eigen::Vector3d neighbor_position = current.position + step;
          if (!edt_env_->sdf_map_->isInBox(neighbor_position) ||
              edt_env_->sdf_map_->getInflateOccupancy(neighbor_position) == 1 ||
              edt_env_->sdf_map_->getOccupancy(neighbor_position) == SDFMap::UNKNOWN)
            continue;

          bool safe = true;
          const double length = step.norm();
          const Eigen::Vector3d direction = step / length;
          for (double distance = 0.1; distance <= length + 1e-2; distance += 0.1) {
            const Eigen::Vector3d checkpoint = current.position + distance * direction;
            if (edt_env_->sdf_map_->getInflateOccupancy(checkpoint) == 1 ||
                edt_env_->sdf_map_->getOccupancy(checkpoint) == SDFMap::UNKNOWN) {
              safe = false;
              break;
            }
          }
          ...
          posToIndex(neighbor_position, neighbor_index);
          if (!visited.emplace(neighbor_index, 1).second) continue;
          if (static_cast<int>(visited.size()) >= node_limit) {
            node_limit_hit = true;
            break;
          }
          frontier.push({ neighbor_index, neighbor_position });
        }
    if (node_limit_hit) break;
  }

  if (node_limit_hit)
    ROS_WARN_THROTTLE(1.0,
        "Known-free connectivity search hit the %d-node limit; unvisited goals stay filtered.",
        node_limit);
  return best_expansion_distance >= 0.0;
}
