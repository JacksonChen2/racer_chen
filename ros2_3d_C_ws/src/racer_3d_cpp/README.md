...
Responsive five-UAV `warehouse_simple` visualization:

```bash
cd RACER/ros2_3d_C_ws
RACER_3D_SCENARIO=warehouse_simple \
RACER_3D_SCENE_USD=../ros2_3d_py_ws/warehouse_simple.usd \
...
physics rate. Three-vehicle visualization refreshes these transforms at 60 Hz;
the five-vehicle profile uses 20 Hz to reserve the UI thread for camera input.
...
The shared occupied map is cyan; up to five vehicles' flown trails and current
plans are red, green, yellow, purple, and orange. The display consumes
`/drone_0/occupied_voxels` and each `/drone_N/planned_path_3d` topic without
changing the map or planner. Only the display is capped: three vehicles use
40,000 occupied points and five vehicles use 12,000 by default. Override it with
...
Interactive mode services the viewport on a 30 Hz wall-clock schedule instead
of waiting for the slower simulation clock. The five offscreen 640x480 depth
render products are paused on UI-only frames and re-enabled on their exact
30 Hz simulation-time sensor frames. This keeps mouse orbit/pan/zoom responsive
without changing RACER's point clouds, map resolution, dynamics, or collision
detection. Override the UI target with `RACER_3D_INTERACTIVE_RENDER_HZ`.
