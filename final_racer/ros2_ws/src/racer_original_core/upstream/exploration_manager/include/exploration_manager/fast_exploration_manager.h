...
public:
  ...
  // Allow an unassigned UAV to keep exploring its nearest reachable local
  // frontier while it waits for the next event-triggered global assignment.
  // The fallback is deliberately opt-in so the original RACER baseline keeps
  // its existing NO_GRID behavior.
  void setUnassignedFrontierFallback(bool enabled) {
    allow_unassigned_frontier_fallback_ = enabled;
  }
  ...
  // Refresh the existing FrontierFinder/HGrid view and return the active grid
  // task ids. This is a read-only coordination adapter around RACER's map and
  // grid data; it does not introduce another frontier representation.
  void updateGlobalGridPool(const Eigen::Vector3d& pos, vector<int>& grid_ids);

  // Reuse HGrid's existing A*/geometric cost estimate for assignment utility.
  double estimateGridTravelCost(const Eigen::Vector3d& pos, int grid_id);
...
private:
  ...
  // Mapping from the compact reachable-only frontier set used in one planning round to the
  // persistent FrontierFinder ids. It is rebuilt for every planning attempt and is never a
  // blacklist.
  vector<int> reachable_frontier_ids_;
  bool allow_unassigned_frontier_fallback_{ false };
