...
class CommunicationProxy final : public rclcpp::Node {
 public:
  CommunicationProxy()
    ...
    task_metric_observer_mode_ = declare_parameter<std::string>(
        "task_metric_observer_mode", "off");
    task_metric_sample_period_s_ = declare_parameter<double>(
        "task_metric_sample_period_s", 0.1);
    ground_truth_occupied_voxels_path_ = declare_parameter<std::string>(
        "ground_truth_occupied_voxels_path", "");
    task_metric_output_path_ = declare_parameter<std::string>(
        "task_metric_output_path", "");
    ...
    if (task_metric_observer_mode_ != "off" &&
        task_metric_observer_mode_ != "async") {
      throw std::runtime_error(
          "task_metric_observer_mode must be off or async");
    }
    if (task_metric_observer_mode_ == "async" &&
        (!std::isfinite(task_metric_sample_period_s_) ||
         task_metric_sample_period_s_ <= 0.0 ||
         ground_truth_occupied_voxels_path_.empty() ||
         task_metric_output_path_.empty())) {
      throw std::runtime_error(
          "async task metrics require a positive sample period, GT path, "
          "and output path");
    }
    ...
    if (task_metric_observer_mode_ == "async") startTaskObserver();
    ...
    RCLCPP_INFO(
        ...
        "waveform=CP-OFDM task_metric_observer=%s sample_period_s=%.3f",
        ...
        task_metric_observer_mode_.c_str(), task_metric_sample_period_s_);
  }

  ~CommunicationProxy() override { stopTaskObserver(); }
 ...
 private:
  ...
  void loadGroundTruthOccupiedVoxels() {
    ...
    if (!input) {
      throw std::runtime_error(
          "cannot open ground-truth occupied voxel file: " +
          ground_truth_occupied_voxels_path_);
    }
    std::uint64_t address{};
    while (input >> address) {
      if (address > std::numeric_limits<std::uint32_t>::max()) {
        throw std::runtime_error("ground-truth voxel exceeds uint32 range");
      }
      ground_truth_occupied_voxels_.insert(
          static_cast<std::uint32_t>(address));
    }
    if (ground_truth_occupied_voxels_.empty()) {
      throw std::runtime_error("ground-truth occupied voxel file is empty");
    }
  }

  void enqueueTaskObservation(
      const std::shared_ptr<rclcpp::SerializedMessage> &message) {
    {
      std::lock_guard<std::mutex> lock(task_observer_queue_mutex_);
      task_observer_queue_.push_back({message});
  ...
  void applyPassiveChunk(
      const racer_fidelity_msgs::msg::ChunkData &chunk) {
    if (chunk.chunk_drone_id < 1 || chunk.chunk_drone_id > drone_count_) return;
    auto &owner_voxels = uav_observed_voxels_[
        static_cast<std::size_t>(chunk.chunk_drone_id - 1)];
    for (const auto address : chunk.voxel_adrs) {
      if (!owner_voxels.insert(address).second) continue;
      ++total_uav_observed_voxels_;
      auto &owners = observed_voxel_owner_counts_[address];
      if (owners++ == 0U) ++unique_uav_observed_voxels_;
    }
    ...
    for (std::size_t index = 0; index < count; ++index) {
      ...
      if (chunk.voxel_occ[index] == 1U) {
        if (bs_occupied_voxels_.insert(address).second &&
            ground_truth_occupied_voxels_.count(address)) {
  ...
  double passiveRedundancy() const {
    if (total_uav_observed_voxels_ == 0U) return 0.0;
    return std::clamp(
        1.0 - static_cast<double>(unique_uav_observed_voxels_) /
                  static_cast<double>(total_uav_observed_voxels_),
        0.0, 1.0);
  }

  double passiveMapIou() const {
    const auto union_size = bs_occupied_voxels_.size() +
  ...
  void writeTaskSamplesThrough(double sim_time_s) {
    if (!std::isfinite(sim_time_s) || sim_time_s < 0.0) return;
    while (next_task_metric_sample_s_ <=
           sim_time_s + 1.0e-9) {
      task_metric_output_
          << std::setprecision(12)
          << "{\"time_s\":" << next_task_metric_sample_s_
          << ",\"redundant_exploration_ratio\":"
          << passiveRedundancy()
          << ",\"bs_global_map_iou\":" << passiveMapIou()
          << "}\n";
      task_metric_output_.flush();
      ++task_metric_samples_written_;
      next_task_metric_sample_s_ += task_metric_sample_period_s_;
    }
  }

  void taskObserverLoop() {
    while (true) {
      std::deque<PassiveTaskObservation> observations;
      bool stopping = false;
      {
        std::unique_lock<std::mutex> lock(task_observer_queue_mutex_);
        task_observer_condition_.wait_for(
            lock, std::chrono::milliseconds(2), [this]() {
              return task_observer_stopping_ ||
                     !task_observer_queue_.empty();
            });
        observations.swap(task_observer_queue_);
        stopping = task_observer_stopping_;
      }
      for (const auto &observation : observations) {
        racer_fidelity_msgs::msg::ChunkData chunk;
        if (observation.serialized &&
            deserialize(observation.serialized, chunk)) {
          applyPassiveChunk(chunk);
        }
        ++task_observer_events_processed_;
      }
      writeTaskSamplesThrough(now().seconds());
      if (stopping && observations.empty()) break;
    }
  }

  void startTaskObserver() {
    loadGroundTruthOccupiedVoxels();
    const std::filesystem::path output_path(task_metric_output_path_);
    if (!output_path.parent_path().empty()) {
      std::filesystem::create_directories(output_path.parent_path());
    }
    task_metric_output_.open(output_path, std::ios::out | std::ios::trunc);
    if (!task_metric_output_) {
      throw std::runtime_error(
          "cannot open passive task metric output: " +
          task_metric_output_path_);
    }
  ...
  void observeUavTransmit(
    ...
    if (task_metric_observer_mode_ == "async") {
      enqueueTaskObservation(message);
    }
  }
  ...
  std::string task_metric_observer_mode_{"off"};
  double task_metric_sample_period_s_{0.1};
  std::string ground_truth_occupied_voxels_path_;
  std::string task_metric_output_path_;
  std::vector<std::unordered_set<std::uint32_t>> uav_observed_voxels_;
  std::unordered_map<std::uint32_t, std::uint16_t>
      observed_voxel_owner_counts_;
  std::unordered_set<std::uint32_t> bs_occupied_voxels_;
  std::unordered_set<std::uint32_t> ground_truth_occupied_voxels_;
  std::size_t total_uav_observed_voxels_{};
  std::size_t unique_uav_observed_voxels_{};
  std::size_t bs_gt_intersection_voxels_{};
  double next_task_metric_sample_s_{};
  std::ofstream task_metric_output_;
  std::mutex task_observer_queue_mutex_;
  std::condition_variable task_observer_condition_;
  std::deque<PassiveTaskObservation> task_observer_queue_;
  ...
  std::uint64_t task_metric_samples_written_{};
