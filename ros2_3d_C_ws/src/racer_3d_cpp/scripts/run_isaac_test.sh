...
interactive_render_hz="${RACER_3D_INTERACTIVE_RENDER_HZ:-30}"
...
if ! python3 -c 'import sys; assert float(sys.argv[1]) > 0' "${interactive_render_hz}"; then
  printf 'RACER_3D_INTERACTIVE_RENDER_HZ must be positive\n' >&2
  exit 2
fi
...
if [[ "${visualize}" == "1" ]]; then
  visualization_args=(
    ...
    --interactive-render-hz "${interactive_render_hz}"
  )
fi
