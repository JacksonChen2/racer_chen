...
bool FrontierFinder::splitHorizontally(const Frontier& frontier, vector<Frontier>& splits) {
  ...
  // Materialize Eigen expressions while their temporary complex matrices are
  // alive.  Storing the lazy expressions with `auto` leaves dangling
  // references and causes optimized builds to crash during frontier splits.
