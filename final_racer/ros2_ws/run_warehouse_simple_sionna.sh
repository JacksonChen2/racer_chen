...
if [[ -n "${RACER_COMM_OVERLAY_SETUP:-}" ]]; then
  # Keep the reference RACER planner/adapter from this workspace while
  # allowing the distributed-radio implementation to live in an isolated
  # overlay.  Source it last so only packages present in that overlay win.
  source "${RACER_COMM_OVERLAY_SETUP}"
fi
set -u
...
coverage_update_rate_hz="${RACER_COVERAGE_UPDATE_RATE_HZ:-}"
task_metric_observer_mode="${RACER_TASK_METRIC_OBSERVER_MODE:-}"
...
if [[ "${initial_assignment_perfect_delivery}" == "true" ]] &&
  ...
  printf 'Initial-assignment perfect delivery requires non-ideal distributed communication.\n' >&2
  exit 2
fi
...
if [[ -n "${coverage_update_rate_hz}" ]] &&
   ! python3 -c 'import math,sys; value=float(sys.argv[1]); raise SystemExit(not (math.isfinite(value) and value > 0.0))' "${coverage_update_rate_hz}"; then
  printf 'RACER_COVERAGE_UPDATE_RATE_HZ must be a positive finite number.\n' >&2
  exit 2
fi
...
if [[ "${exploration_assignment_mode}" != "original" &&
  ...
  printf 'RACER_EXPLORATION_ASSIGNMENT_MODE must be original, local_component, or global_cooperative.\n' >&2
  exit 2
fi
if [[ "${exploration_assignment_mode}" == "local_component" &&
      "${initial_assignment_perfect_delivery}" != "false" ]]; then
  printf 'local_component requires RACER_INITIAL_ASSIGNMENT_PERFECT_DELIVERY=false.\n' >&2
  exit 2
fi
...
launch_communication_args=(
  ...
  map_perfect_delivery:="${RACER_MAP_PERFECT_DELIVERY:-false}"
...
if [[ -n "${coverage_update_rate_hz}" ]]; then
  launch_communication_args+=(
    coverage_update_rate_hz:="${coverage_update_rate_hz}"
  )
fi
if [[ -n "${task_metric_observer_mode}" ]]; then
  launch_communication_args+=(
